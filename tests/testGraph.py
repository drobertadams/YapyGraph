...
class TestGraphClass(unittest.TestCase):
    ...
    def testFindVertex(self):
        self.assertIsNone(self.g.getVertex('X'))       # there is no "X" in empty graph

        # Add a single vertex with name "A1".
        u1 = self.g.addVertex(Vertex('u1', 'A', 1))

        self.assertIsNone(self.g.getVertex('X'))       # there is no "X"
        self.assertIsNone(self.g.getVertex('A2'))      # there is no "A2"
        self.assertIsNone(self.g.getVertex('X1'))      # there is no "X1"
        self.assertEqual(self.g.getVertex('A1'), u1)   # but there is an A1
